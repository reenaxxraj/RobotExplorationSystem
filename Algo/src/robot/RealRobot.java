...
public class RealRobot extends Robot {
	...
	protected String[] getSensorValues() {
		...
		Pattern sensorPattern2 = Pattern.compile(
				"\\d+[.]\\d+[,]{1}\\d+[.]\\d+[,]{1}\\d+[.]\\d+[,]{1}\\d+[.]\\d+[,]{1}\\d+[.]\\d+[,]{1}\\d+[.]\\d+");
		String s;
		String[] arr = null;
	...
	public void forward(int step) {
		connectionSocket.sendMessage(Integer.toString(step));
	...
	public void UTurn() {
		connectionSocket.sendMessage(Constant.UTurn);
		setDirection((this.getDirection() + 3) % 4);
		if (sr != null) {
			sr.rotateLeft();
			sr.displayMessage("Sent message: " + Constant.TURN_LEFT, 1);
	...
	public boolean captureImage(int[][] image_pos) {
		connectionSocket.sendMessage("C[" + image_pos[0][1] + "," + image_pos[0][0] + "|" + image_pos[1][1] + ","
				+ image_pos[1][0] + "|" + image_pos[2][1] + "," + image_pos[2][0] + "]"); // This is left middle right.
																							...
																							// Real Run.
		System.out.println("Capture image at : " + "C[" + image_pos[0][1] + "," + image_pos[0][0] + "|"
				+ image_pos[1][1] + "," + image_pos[1][0] + "|" + image_pos[2][1] + "," + image_pos[2][0] + "]");
		if (sr != null) {
